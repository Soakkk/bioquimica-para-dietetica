...
export default function InlineLessonPractice({
  ...
  return <section className="inline-practice" id="lesson-practice">
    ...
    <article className="inline-question">
      ...
      {feedback && <div className={`inline-feedback ${feedback}`}><span>{feedback === "correct" ? "✓" : "↺"}</span><div><b>{feedback === "correct" ? "Correcto · concepto afianzado" : "Revisa el paso que has aplicado"}</b><p>{current.explain}</p></div></div>}

      <footer><button disabled={index === 0} onClick={() => go(index - 1)}>← Anterior</button><button onClick={() => setHint(true)}>✦ Pista</button><button onClick={() => onAsk(`Estoy en la lección “${moduleTitle}”. Ayúdame a razonar este ejercicio sin darme directamente la respuesta: ${current.prompt} ${current.formula}`)}>Preguntar al tutor</button>{current.options.length > 1 && revealed && (feedback === "correct" ? <button className="primary" onClick={() => go(index === questions.length - 1 ? 0 : index + 1)}>{index === questions.length - 1 ? "Repetir bloque" : "Siguiente"} →</button> : <button className="primary" disabled={picked === null} onClick={check}>Comprobar</button>)}</footer>
    </article>
  </section>;
}
